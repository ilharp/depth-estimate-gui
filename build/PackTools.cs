    ...
    Target PackTools => _ => _
        ...
        .Executes(() =>
        {
            ...
            ProcessTasks.StartShell(
                    $"{(Platform is (PlatformFamily.Linux or PlatformFamily.OSX) ? "sudo " : "")}conda create -y -p . python=3.7 pytorch torchvision {(CudaVersion == "cpuonly" ? CudaVersion : "cudatoolkit=" + CudaVersion)} opencv matplotlib timm -c pytorch-lts -c conda-forge",
                    ToolsDirectory)
                .AssertZeroExitCode();
